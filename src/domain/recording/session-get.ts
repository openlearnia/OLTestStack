...
import { getSessionDetail } from '../../dashboard/queries.js';
import type { SessionDetailResponse } from '../../dashboard/types.js';
import { z } from 'zod';
...
export async function getPersistedSession(
  ctx: AppContext,
  input: unknown,
): Promise<McpSuccessResponse<SessionDetailResponse> | McpErrorResponse> {
  const parsed = sessionGetSchema.safeParse(input);
  if (!parsed.success) {
  ...
  const detail = await getSessionDetail(db, reportId);
